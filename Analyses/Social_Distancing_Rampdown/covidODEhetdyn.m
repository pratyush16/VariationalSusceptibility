function dsdt = covidODEhetdyn(t,s)
    ...
    global lam de ga rh N xvec qvec k1 k2 drampup drampdown dmax p inidist R0 i data
    ...
    sus = 1 + prox*(xvec-1);
    behetcon = R0(i)/(rh/de+1/ga)*k1/k2;
    lam = behetcon*prox/data(i).pop*sus*(rh*s(N+1:2*N)+s(2*N+1:3*N))/(sus*qvec);
 
    dsdt(1:N)       = - lam*sus'.*s(1:N);
    dsdt(N+1:2*N)   =   lam*sus'.*(s(1:N)) - de*s(N+1:2*N);
